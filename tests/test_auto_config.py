...
class TestCEConfigDefaults:
    ...
    def test_pose_config_has_ce_fields(self, cuda_hw):
        """Pose config values include label_smoothing and mixup_alpha."""
        cfg = auto_config.build_config_values("pose", 100, "mid", cuda_hw)
        assert "label_smoothing" in cfg
        assert "mixup_alpha" in cfg
